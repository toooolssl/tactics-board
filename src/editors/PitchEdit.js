...
class PitchEdit extends Component {
  ...
  objectDrag(posX, posY, deltaX, deltaY, snap) {
    ...
    switch (this._dragObjectType) {
      ...
      default:
        console.log(
          "Invalid drag object type",
          this._dragObjectType,
          this._dragNode
        );
        break;
    }
    ...
    return balls.map((b, index) => {
      return (
        <BallEdit
          ...
          isEdit={isEdit}
          isPrevFrame={isPrevFrame}
          ballHref={this._appConfig.ballHref}
        />
      );
    });
  }
  ...
  renderSquares(squares) {
    return squares.map((s, index) => {
      return <SquareEdit key={index.toString()} square={s} />;
    });
  }

  renderEllipses(ellipses) {
    return ellipses.map((el, index) => {
      return <EllipseEdit key={index.toString()} ellipse={el} />;
    });
  }
  ...
  render() {
    ...
    // Clase del contenedor con el offset para el menú
    const pitchClasses = "pitch " + this.props.classes.offset;

    // Obtener la posición del pitch en el viewBox
      ...
      <React.Fragment>
        <div ref={this._editRef} className={pitchClasses}>
          ...
          >
            ...
            <g id="ellipses">
              {this.renderEllipses(this.props.pitch.ellipses)}
            </g>
            ...
            <g id="players" fontSize="50">
              {this.renderLines(
                this.props.pitch.playerPathsCurrentKeyFrame(),
                true,
                true
              )}
              {this.renderPlayers(
                this.props.pitch.playersPreviousKeyFrame(),
                false,
                true
              )}
              {this.renderPlayers(
                this.props.pitch.playersCurrentKeyFrame(),
                true,
                false
              )}
              {this.renderLines(
                this.props.pitch.AnimAllPlayerPaths(),
                true,
                true
              )}
            ...
            <g id="balls">
              {this.renderLines(
                this.props.pitch.ballPathsCurrentKeyFrame(),
                true,
                false
              )}
              {this.renderBalls(
                this.props.pitch.ballsPreviousKeyFrame(),
                false,
                true
              )}
              {this.renderBalls(
                this.props.pitch.ballsCurrentKeyFrame(),
                true,
                false
              )}
              {this.renderLines(
                this.props.pitch.AnimAllBallPaths(),
                true,
                false
              )}
        ...
        <ContextMenu
          ...
          onClose={this.hContextMenuClose}
        />
      </React.Fragment>
    );
  }
